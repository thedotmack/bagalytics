    ...
    <div className="dark min-h-screen bg-zinc-950 text-white">
      ...
      <div className="relative z-10 max-w-6xl mx-auto pt-6 px-4 sm:px-6 lg:px-8">
        ...
        <header className="flex flex-col gap-3 sm:gap-4">
          {/* Top row: Logo, Social icons, GitHub stars */}
          <div className="flex items-center justify-between">
            ...
            {/* Right: Social links + GitHub stars */}
            <div className="flex items-center gap-1 sm:gap-2">
              {/* X (Twitter) - visible on all sizes, smaller on mobile */}
              <a
                href="https://x.com/Claude_Memory"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 text-zinc-500 hover:text-emerald-400 transition-colors"
                title="Follow on X"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="sm:w-[18px] sm:h-[18px]">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              {/* Discord - visible on all sizes, smaller on mobile */}
              <a
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 sm:gap-1.5 h-8 sm:h-10 px-2 sm:px-3 text-zinc-500 hover:text-emerald-400 transition-colors"
              ...
              >
                ...
                <span className="text-xs sm:text-sm font-medium">{githubStars !== null ? (githubStars >= 1000 ? `${(githubStars / 1000).toFixed(1)}k` : githubStars) : "—"}</span>
              </a>
            </div>
          </div>

          {/* Search row - full width on mobile, inline on larger screens */}
          <div className="flex gap-2">
            <Input value={tokenCA} onChange={(e) => setTokenCA(e.target.value)} placeholder="Enter token contract address..." className="flex-1 sm:w-64 md:w-80 lg:w-96 h-9 px-3 text-sm bg-zinc-900 border-zinc-700 rounded-lg font-mono placeholder:text-zinc-500 focus:border-neon-500 focus:ring-neon-500/20" />
            <Button onClick={analyze} disabled={loading} className="h-9 px-3 sm:px-4 text-sm rounded-lg bg-neon-600 hover:bg-neon-500 font-semibold text-black whitespace-nowrap">
              bagalyze
            </Button>
          </div>
        </header>
